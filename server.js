...
const STORAGE_DRIVER = text(process.env.DATABASE_URL) ? "database_configured_pending_adapter" : "local_json";
...
function adminToken() {
  return text(process.env.HARITO_ADMIN_TOKEN);
}
...
function paymentProviderStatus() {
  if (text(process.env.RAZORPAY_KEY_ID) && text(process.env.RAZORPAY_KEY_SECRET)) {
    return {
      provider: "razorpay",
      configured: true,
  ...
  if (text(process.env.HARITO_UPI_ID)) {
    return {
      provider: "manual_upi",
      configured: true,
      liveCollectionEnabled: process.env.HARITO_ENABLE_LIVE_PAYMENTS === "true",
...
function paymentConfiguration(total, orderRef) {
  ...
  if (provider.provider === "razorpay") {
    return {
      provider: "razorpay",
      status: provider.liveCollectionEnabled ? "gateway_order_pending" : "provider_configured_not_live",
      upiIntent: null,
      instructions: provider.liveCollectionEnabled
        ? "Create a Razorpay order before redirecting the buyer to payment."
        : "Razorpay keys are configured, but live collection is disabled until launch approval."
    };
  }
...
async function handleApi(request, response, url) {
  ...
  if (url.pathname === "/api/payments/config" && request.method === "GET") {
    sendJson(response, 200, paymentProviderStatus());
    return true;
  }
  ...
  if (url.pathname === "/api/admin/session" && request.method === "POST") {
    ...
    if (!adminTokenRequired() || token === adminToken()) {
      sendJson(response, 200, { ok: true, adminTokenRequired: adminTokenRequired() });
      return true;
    }
    sendError(response, 401, "Admin token invalid.");
    return true;
  }
